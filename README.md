...
## krilld — the host agent (M1, current)
...
runnable from [`m1-gates/`](m1-gates/).

## Roadmap

| Milestone | What | Status |
|---|---|---|
| M1 | `krilld`: registry, VMM driver, lifecycle, wake router | **in progress** |
| M2 | deploy path: MCP server + CLI, dir → rootfs → URL | |
| M3 | data plane: host-side WAL shipping, epoch fencing as code, PITR — the TLA+ spec becomes the test oracle | |

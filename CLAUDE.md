...
them. **M1, M2 and M3 are complete and accepted (2026-07-23), and M4 — the doorman
— is built but NOT YET GATED (2026-07-26): the code, the gate scripts and the
exposure runbook exist; F1–F7 have not been run on hardware.**
...
| `SERVER-SETUP.md` | Runbook for the production Hetzner EX44 (live since 2026-07-26): rescue → RAID1 installimage → hardening → FC v1.16.1 → krilld systemd unit → tunnel-based deploys → **Phase 7: the objstore on GCS + nightly registry backups (all seven phases executed and verified).** **Posture: SSH-only inbound, router+admin on loopback. Phase 9 (the doorman, Caddy, and first exposure) is written but NOT YET RUN.** ⚠ Never repoint `--objstore` without copying the record first — an empty store means an empty stream (E4) and the next wake wipes `/data`. |
...
| `internal/objstore` + `internal/sqlitewal` + `internal/ext4` + `internal/dataplane` | **The M3 data plane: rules E1–E6 as code.** Object store with conditional PUT (fsstore/GCS/mem, E4) — GCS auth resolves `KRILL_GCS_TOKEN` → service-account JSON (`gcsauth.go`, the non-GCP production path) → GCE metadata → `gcloud`; `objstore.Check` is the startup preflight that proves CAS works (including that a **stale** CAS is rejected), `objstore.Copy` moves the record between backends; SQLite WAL parse/replay at commit boundaries; read-only ext4 **with jbd2 journal replay** (host tails the guest's WAL from outside the VM); epochs/manifest/gateway/shipper/restore/PITR-branching + the lifecycle Coordinator. `internal/dataplane/sim` is the deterministic-simulation harness: **the TLA+ spec run as the test oracle against the real code** — its three negative configs must keep reproducing the TLC counterexamples (PT-1, E6 bug, PT-9). |
...
| `internal/regbackup` | Ships the registry (the E1 epoch mint + app catalog — the only host state nothing else can reconstruct) to the object store: `VACUUM INTO` snapshot → gzip → `_control/registry/<ts>.db.gz` + a sidecar recording `cell_gen` and the minimum safe `restore_cell_gen`. Age-driven schedule, so `Restart=always` can't spray backups. |
| `internal/doorman` + `cmd/krill-doorman/` | **M4's front door.** Unprivileged process behind Caddy's `forward_auth`: only a 200 from it lets a request reach krilld's router (the only thing that can wake an app). Google OIDC (bought: `x/oauth2` + `coreos/go-oidc`), opaque per-app sessions over a master session on `auth.<base-host>`, ed25519 identity tokens with `aud` = one app, the three-plane ACL, share links as capabilities, and **revocation as tombstones in an append-only object-store log** — written before a revoke is acked, replayed at every start, so no restore can resurrect access (F2). Control API on admin port + 1. |
| `internal/egress` | **F6's baseline as one nftables table**, applied in a single transaction: apps silent, builder VMs reaching a named allowlist and nothing else, nobody reaching another guest / the host / link-local / SMTP, everything permitted rate-limited and counted. `--egress-build-allow` is the deliberate widening for Dockerfiles that run `apt-get`/`pip`. |
| `internal/buildvm` + `m4-gates/builder-image/` | **F5: `docker build` of an untrusted context runs inside a throwaway microVM**, context in on a read-only disk, golden image out on the disk it populates, teardown on a `defer`. `internal/admin` refuses network-arriving deploys when no builder VM exists — there is no host fallback, because the fallback is the vulnerability. |
| `deploy/Caddyfile` | The annotated production front-door config: DNS-01 wildcard, identity headers stripped inbound, `forward_auth` to the doorman, session cookie stripped on the way to the guest. |
| `m4-gates/` | Pre-registered F1–F7 gates (`GATES.md` frozen before code; F2 amended once, a tightening) + runnable scripts, `RESULTS-TEMPLATE.md`, and the `watchlist` (F4) and `hostile` (F5) examples. **Not yet run.** |
| `cmd/krill/` | The CLI: `deploy` (dir → URL, verified), `apps/status/logs/wake/freeze/delete/stream/restore`, plus the durability verbs — `durability` (live CAS check + backups that actually landed), `backup` (snapshot the mint now), `objstore-copy` (move the record between stores; the required first step of any `--objstore` change). Thin admin-API client except `objstore-copy`, which talks to the stores directly so it works with krilld stopped; flags parse in any position (`parseFlexible`). |
| `mcp/` | TypeScript MCP server (the ROADMAP's language decision): `deploy`/`logs`/`apps`/`delete_app` over stdio (thin client of the admin API) plus M4's `share`/`shares`/`unshare` against the doorman (`KRILL_DOORMAN`, default admin port + 1). `npm install && npm run build` → `dist/index.js`; see `mcp/README.md` for `claude mcp add`. |
...
- **M4's one-line summary of where authorization lives:** the doorman decides,
  strictly upstream of krilld, and a refusal there means no wake happened. If
  you are ever tempted to move an auth check into krilld's router, re-read
  ROADMAP decision #9 first — F3 forbids an unauthorized request from waking
  an app, and the router wakes on request.

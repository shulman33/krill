...
| `SERVER-SETUP.md` | Runbook for the production Hetzner EX44 (live since 2026-07-26): rescue → RAID1 installimage → hardening → FC v1.16.1 → krilld systemd unit → tunnel-based deploys → **Phase 7: the objstore on GCS + nightly registry backups (all seven phases executed and verified).** **Posture: SSH-only inbound, router+admin on loopback until M4 ships the doorman.** ⚠ Never repoint `--objstore` without copying the record first — an empty store means an empty stream (E4) and the next wake wipes `/data`. |
